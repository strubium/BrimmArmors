...
import interactive.blackout.brimm.loader.obj.ObjModelLoader;
...
public class AdvancedModelLoader {
    ...
    public static IModelCustom loadModel(ResourceLocation resource) throws ModelFormatException, IllegalArgumentException {
        ...
        if (i == -1) {
            BrimmArmors.LOGGER.error("The resource name %s is not valid", resource);
            throw new IllegalArgumentException("The resource name is not valid");
        } else {
            String suffix = name.substring(i + 1);
            IModelCustomLoader loader = instances.get(suffix);
            if (loader == null) {
                BrimmArmors.LOGGER.error("The resource name %s is not supported", resource);
